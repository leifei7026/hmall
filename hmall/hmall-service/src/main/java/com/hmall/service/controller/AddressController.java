package com.hmall.service.controller;

import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.hmall.api.dto.PageDTO;
import com.hmall.api.entity.Address;
import com.hmall.service.service.AddressService;
import io.swagger.v3.oas.annotations.Operation;
import io.swagger.v3.oas.annotations.Parameter;
import io.swagger.v3.oas.annotations.tags.Tag;
import org.springframework.web.bind.annotation.*;

import javax.annotation.Resource;

@Tag(name = "收货地址管理", description = "地址增删改查接口")
@RestController
@RequestMapping("/api/address")
public class AddressController {

    @Resource
    private AddressService addressService;

    @Operation(summary = "根据ID查询地址")
    @GetMapping("/{id}")
    public Address getById(@Parameter(description = "地址ID") @PathVariable Long id) {
        return addressService.getById(id);
    }

    @Operation(summary = "分页查询地址")
    @GetMapping("/list")
    public PageDTO<Address> list(
            @Parameter(description = "页码", example = "1") @RequestParam(defaultValue = "1") Integer pageNum,
            @Parameter(description = "每页大小", example = "10") @RequestParam(defaultValue = "10") Integer pageSize) {
        return PageDTO.of(addressService.page(new Page<>(pageNum, pageSize)));
    }

    @Operation(summary = "新增地址")
    @PostMapping
    public Address save(@RequestBody Address address) {
        addressService.save(address);
        return address;
    }

    @Operation(summary = "更新地址")
    @PutMapping
    public Address update(@RequestBody Address address) {
        addressService.updateById(address);
        return address;
    }

    @Operation(summary = "根据ID删除地址")
    @DeleteMapping("/{id}")
    public Boolean deleteById(@Parameter(description = "地址ID") @PathVariable Long id) {
        return addressService.removeById(id);
    }
}
